...
DATASET_DESC = (
    "This dataset is composed of fully-deuterated Gd(III) analogue d-[GdL] in a "
    "variety of solvent materials, including MeOH, D2O and d6-DMSO."
)
ELEMENTS = None
GLOB_STR = "OUTCAR"
...
CSS = [
    [
        f"{DATASET_NAME}_MeOH",
        {"names": {"$regex": "meoh"}},
        f"Configurations of d-GdL solvated in MeOH from {DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_DMSO",
        {"names": {"$regex": "dmso"}},
        f"Configurations of d-GdL solvated in DMSO from {DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_D2O",
        {"names": {"$regex": "h2o"}},
        f"Configurations of d-GdL solvated in D2O from {DATASET_NAME} dataset",
    ],
]
    ...
    with open(fp, "r") as f:
        ...
        for line in f:
            ...
            # handle coords/nums
                ...
                config.info["name"] = f"{'__'.join(fp.parts[-3:-1])}"
