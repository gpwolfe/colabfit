"""
MAD-1.5_r2SCAN_Test ingest script for ColabFit VAST database.
...
File notes
----------
Source: mad15/mad-1.5-r2scan-test.xyz
...
smearing of 0.05 eV. The test split (~10%) uses a stratified split consistent
with the training and validation splits, excluding monomers, dimers, and trimers
(which are fixed in the training split as physical constraints).
...
loader.config_table = "ndb.colabfit.dev.co_mad15_r2sc_tst"
loader.config_set_table = "ndb.colabfit.dev.cs_mad15_r2sc_tst"
loader.dataset_table = "ndb.colabfit.dev.ds_mad15_r2sc_tst"
loader.co_cs_map_table = "ndb.colabfit.dev.cs_co_map_mad15_r2sc_tst"

DATASET_ID = "DS_7q71mf99le0c_0"
DATASET_NAME = "Massive_Atomic_Diversity_MAD-1.5_r2SCAN_Test"
DESCRIPTION = (
    "Test split of the MAD-1.5 (Massive Atomic Diversity version 1.5) dataset, "
    ...
    "uncertainty-based filtering. The test split (~10% of cleaned data, excluding "
    "monomers, dimers, and trimers which are fixed in the training split) uses a "
    "stratified split method consistent with the training and validation splits. "
    "Subset-resolved MAE for PET-MAD-1.5-S on this test set is 11.09 meV/atom "
    "(energy) and 36.81 meV/Angstrom (forces). A companion PBE-functional dataset "
    "(Massive_Atomic_Diversity_MAD-1.5_PBE) was used during model training with separate prediction heads. "
)
...
DATA_PATH = Path(__file__).parent / "mad-1.5-r2scan-test.xyz"

SUBSET_DESCRIPTIONS = {
    "mc3d": (
        "Bulk crystals from the Materials Cloud 3D (MC3D) database, "
        "part of the MAD-1.5_r2SCAN_Test dataset."
    ),
    "mc3d_rattled": (
    ...
    "mc3d_random": (
        ...
        "structures, part of the MAD-1.5_r2SCAN_Test dataset."
    ),
    "mc3d_surface": (
    ...
    "trimers": (
        ...
        "102 elements, part of the MAD-1.5_r2SCAN_Test dataset."
    ),
    "mc3d_ext": (
        "Extended MC3D crystals covering 93 elements with Z<100, including "
        "lanthanides and actinides, part of the MAD-1.5_r2SCAN_Test dataset."
    ),
    "mc3d_random_ext": (
