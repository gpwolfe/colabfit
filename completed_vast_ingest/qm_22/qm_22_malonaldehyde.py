...
loader.config_table = "ndb.colabfit.dev.co_qm22_malon"
loader.prop_object_table = "ndb.colabfit.dev.po_qm22_malon"
loader.config_set_table = "ndb.colabfit.dev.cs_qm22_malon"
loader.dataset_table = "ndb.colabfit.dev.ds_qm22_malon"
...
DATASET_FP = Path(
    "/scratch/gw2338/vast/data-lake-main/spark/scripts/gw_scripts/qm22/Malonaldehyde.xyz"  # noqa: E501
)
DATASET_NAME = "QM-22_Malonaldehyde"

DATASET_ID = "DS_5yp2nd8n5nau_0"
LICENSE = "Apache-2.0"
PUBLICATION = "https://doi.org/10.1063/1.2937732"
...
DESCRIPTION = "The Malonaldehyde set of the QM-22 datasets, with energies calculated at the CCSD(T) level of theory. QM-22 consists of CHON molecules of 4-15 atoms, developed in counterpoint to the MD17 dataset, run at higher total energies (above 500 K) and with a broader configuration space."  # noqa: E501

property_map = PropertyMap([energy_pd])
property_map.set_metadata_field("software", "MOLPRO")
property_map.set_metadata_field("method", "CCSD(T)")
property_map.set_metadata_field("basis-set", "aug-cc-pVTZ")
property_map.set_metadata_field("input", "")
property_map.set_metadata_field("reference_energy", "e_ref")
property_map.set_metadata_field("correction_energy_ccsd", "e_corr_ccsd")
property_map.set_metadata_field("correction_energy_t", "e_corr_t")
...
def reader(filepath):
    ...
    with open(filepath, "r") as f:
        for line in f:
            ...
            elif len(l_parts) == 4 and "." in l_parts[0]:
                e_ref, e_corr_ccsd, e_corr_t, energy = [float(x) for x in l_parts]
            elif len(l_parts) == 4:
                s, x, y, z = l_parts
                symbols.append(s)
