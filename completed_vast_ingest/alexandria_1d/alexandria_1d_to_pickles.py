...
def process_batch(fp: Path):
    ...
    configs = []
    file_ix = 0
    output_dir = Path("alexandria_1d_pickles/alexandria_1d")
    ...
    for ix, id in enumerate(data):
        ...
        for j, trajectory in enumerate(val):
            ...
            for i, step in enumerate(trajectory["steps"]):
                ...
                config.info["_name"] = (
                    f"alexandria_1d__file_{fp.stem}__id_{id}__trajectory_{j}__frame_{i}"
                )
                ...
                if len(configs) == 10000:
                    output_path = output_dir / f"alexandria_1d_{fp.stem}_{file_ix}.pkl"
