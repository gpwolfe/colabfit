...
loader.config_table = "ndb.colabfit.dev.co_omol_neutral_train_mp"
loader.prop_object_table = "ndb.colabfit.dev.po_omol_neutral_train_mp"
loader.config_set_table = "ndb.colabfit.dev.cs_omol_neutral_train_mp"
loader.dataset_table = "ndb.colabfit.dev.ds_omol_neutral_train_mp"
loader.co_cs_map_table = "ndb.colabfit.dev.cs_co_map_omol_neutral_train_mp"
...
DATASET_FP = Path(
    "/scratch/gw2338/vast/data-lake-main/spark/scripts/gw_scripts/omol/neutral_train"  # noqa
)
DATASET_NAME = "OMol25_train_neutral"
DATASET_ID = "DS_izdsom1hklzv_0"
DESCRIPTION = "The Train neutral set from OMol25. From the dataset creator: OMol25 represents the largest high quality molecular DFT dataset spanning biomolecules, metal complexes, electrolytes, and community datasets. OMol25 was generated at the ω B97M-V/def2-TZVPD level of theory."  # noqa
DOI = None
...
def read_wrapper(fdir):
    fps = sorted(list(fdir.glob("*.aselmdb")))
    if SLURM_TASK_ID >= len(fps):
        raise ValueError(
            f"SLURM_TASK_ID {SLURM_TASK_ID} is out of range for the number of files {len(fps)}."  # noqa
        )
    logging.info(f"SLURM_TASK_ID: {SLURM_TASK_ID}")

    fp = fps[SLURM_TASK_ID]
