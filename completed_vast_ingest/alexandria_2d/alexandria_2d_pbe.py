...
DATASET_FP = Path(
    "/scratch/gw2338/vast/data-lake-main/spark/scripts/gw_scripts/alex2d/alexandria_2d_pickles"  # noqa
)
DATASET_NAME = "Alexandria_geometry_optimization_paths_PBE_2D"
DATASET_ID = "DS_6pieq95jrqpn_0"
DESCRIPTION = "The Alexandria Materials Database contains theoretical crystal structures in 1D, 2D and 3D discovered by machine learning approaches using DFT with PBE, PBEsol and SCAN methods. This dataset represents the geometry optimization paths for 2D crystal structures from Alexandria calculated using PBE methods."  # noqa
...
def read_wrapper(dir_path: str, start: int, end: int):
    dir_path = Path(dir_path)
    if not dir_path.exists():
        logging.info(f"Path {dir_path} does not exist")
        return
    all_json_paths = sorted(list(dir_path.glob("*.pkl")), key=lambda x: x.stem)
    if start > len(all_json_paths):
        logging.info(
            f"Start index {start} is greater than number of files {len(all_json_paths)}"
        )
        return
    if end > len(all_json_paths):
        end = len(all_json_paths)
    json_paths = all_json_paths[start:end]
    for path in json_paths:
        css_dir = Path("alexandria_2d_css")
...
def main(start, end):
    t0 = time()
    config_generator = read_wrapper(DATASET_FP, start, end)
