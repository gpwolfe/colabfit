...
for pi in tqdm(db.property_instances.find()):
    ...
    pi_type = pi.get("type")
    if rels:
        dos = rels.get("data_objects")
        mds = rels.get("metadata")
        if dos and mds:
            for do_id in dos:
                do = db.data_objects.find_one(
                    {"colabfit-id": do_id}, {"relationships.datasets": 1}
                )
                if do.get("relationships") and do["relationships"].get("datasets"):
                    if do["relationships"]["datasets"][0] in ds_ids:
                        ...
                        meth = md.get("method")
                        if meth:
                            methods[f"{pi_type}__{meth['source-value']}"] += 1
                        soft = md.get("software")
                        if soft:
                            software[f"{pi_type}__{soft['source-value']}"] += 1
with open("methods_no_oc.txt", "w") as f:
    f.write(str(methods))
with open("software_no_oc.txt", "w") as f:
