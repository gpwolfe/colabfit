...
loader.config_table = "ndb.colabfit.dev.co_qm22_tetra_test"
loader.prop_object_table = "ndb.colabfit.dev.po_qm22_tetra_test"
loader.config_set_table = "ndb.colabfit.dev.cs_qm22_tetra_test"
loader.dataset_table = "ndb.colabfit.dev.ds_qm22_tetra_test"
...
DATASET_FP = Path(
    "/scratch/gw2338/vast/data-lake-main/spark/scripts/gw_scripts/qm22/C14H30/testing"  # noqa: E501
)
DATASET_NAME = "QM-22_n-tetradecane_test"

DATASET_ID = "DS_vv60rbjjwpm6_0"
LICENSE = "Apache-2.0"
PUBLICATION = "https://doi.org/10.1021/acs.jctc.4c00932"
...
property_map = PropertyMap([atomic_forces_pd])
...
property_map.set_metadata_field("energy-difference", "energy_diff", dynamic=True)
...
property_map.set_properties([atomic_forces_info])
                ...
                if len(forces) == natoms:
                    yield AtomicConfiguration(
                        ...
                        info={
                            ...
                            "forces": forces,
                            "_name": f"{DATASET_NAME}__file_{filepath.stem}__{count}",
                        },
                    )
                    count += 1
                    symbols = []
...
def read_wrapper(filepath):
    files = sorted(list(filepath.glob("*.xyz")))
    for file in files:
