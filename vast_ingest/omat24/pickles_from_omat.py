...
def process_batch(batch, dataset, dbname, batch_ix):
    ...
    for id, config in id_atom:
        config.info["_name"] = f"OMat24__{config.info['sid']}__id_{id}"
    ...
    output_dir = Path(f"omat_train_pickles/omat24_train_{dbname}")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / f"omat24_train_{dbname}_{batch_ix}.pkl"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python pickles_from_omat.py <filepath>")
        sys.exit(1)
