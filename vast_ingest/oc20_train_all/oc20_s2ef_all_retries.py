...
# loader.config_table = "ndb.colabfit.dev.co_ocall"
# loader.prop_object_table = "ndb.colabfit.dev.po_ocall"
# loader.config_set_table = "ndb.colabfit.dev.cs_ocall"
# loader.dataset_table = "ndb.colabfit.dev.ds_ocall"
# loader.co_cs_map_table = "ndb.colabfit.dev.cs_co_map_ocall"
...
DATASET_FP = Path("/scratch/work/martiniani/for_gregory/vast_oc20")
DATASET_NAME = "OC20_S2EF_train_all"
DATASET_ID = "DS_jyuwhl30jklq_0"
DOI = None
...
DESCRIPTION = (
    "OC20_S2EF_train_all is the ~63 million structure full training set of the OC20 "
    "Structure to Energy and Forces (S2EF) dataset. Features include energy, "
...
def oc_wrapper(dir_path: str, actual_index):
    ...
    if actual_index > len(xyz_paths):
        raise ValueError(f"This index {actual_index} is greater than number of files")
    xyz_path = xyz_paths[actual_index]
    assert xyz_path.stem == str(actual_index)
    print(f"\nProcessing file: {xyz_path}\n")
...
def main(indices):
    beg = time()
    print(beg)
    if len(indices) < SLURM_TASK_ID:
        raise ValueError(f"Index {SLURM_TASK_ID} is out of range")
    actual_index = indices[SLURM_TASK_ID]
    config_generator = oc_wrapper(DATASET_FP, actual_index)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ixs", type=int, nargs="+")
    argv = sys.argv[1:]
