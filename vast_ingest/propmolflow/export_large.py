"""
Targeted export for datasets not included in the main colabfit database, but included
on vast tables
"""
...
def save_po_ids(ds_id):
    ...
    with sess.transaction() as tx:
        table = tx.bucket("colabfit").schema("dev").table("po_pmf")
        reader = table.select(
...
def get_cos(co_ids):
    session = get_session()
    with session.transaction() as tx:
        table = tx.bucket("colabfit").schema("dev").table("co_pmf")
        reader = table.select(
    ...
    df = spark.createDataFrame(
        data,
        StructType([field for field in config_schema]),
    )
    return df
...
def get_pos_df(batch):
    pos = spark.createDataFrame(
        batch.to_struct_array().to_pandas(),
        StructType([field for field in property_object_schema]),
    )
    pos = pos
    pos = pos.select(
        [
...
def merge_po_co_rows(pos, cos):
    ...
    pos = pos.drop(*cols_duplicd, *cols_to_drop_both)
...
def process_ds_id(ds_id):
    ...
    ############################################
    dataset_df = spark.table("ndb.colabfit.dev.ds_pmf").filter(sf.col("id") == ds_id)
