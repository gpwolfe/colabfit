...
File notes
----------
Symbols determined by finding a CONTCARini, POSCARini or POSCARn (or similar)
file. These are explicitly mapped.

excluding structs from files in x0 or x1 folders, as these do not
reliably have defined atom types
"""
...
DATASET_FP = Path(
    "data/analysis_of_minerals_as_electrode_materials_for_ca-"
    "based_rechargeable_batteries"
)
DATASET_NAME = "electrode_materials_for_ca-based_rechargeable_batteries"
...
DATASET_DESC = (
    'Dataset for "Analysis of minerals as electrode materials for Ca-based '
    'rechargeable batteries". Includes DFT structures of pyroxenes, garnet '
    "and carbonates. Dataset was produced to pursue identification of Ca-based "
...
symbol_map = {
    "camni2o6": ["Ca", "Mn", "C", "O"],
    "crgarnet": ["Ca", "Cr", "Si", "O"],
    "icsd27388": ["Ca", "Mn", "Si", "O"],
    "celdaMn112": ["Ca", "Mn", "Si", "O"],
    "dolomita": ["Ca", "Mn", "C", "O"],
    "202248": ["Ca", "Mn", "C", "O"],
}
...
CSS = [
    [
        ...
        {"names": {"$regex": f"{elem}"}},
        f"{elem.capitalize()} configurations from {DATASET_NAME} dataset",
    ]
    for elem in ["piroxene", "garnet", "carbonates"]
]
    ...
    with open(fp, "r") as f:
        ...
        for line in f:
            ...
            # handle lattice
                ...
                # In case keys include "X" instead of "XX", etc.
                for key in stress_keys:
                    if len(key) == 1:
                        stress_keys[stress_keys.index(key)] = key * 2
...
def reader(filepath: Path):
    fps = list(filepath.parent.rglob("OUTCAR"))
    for fp in fps:
        if "NEB" not in "".join(fp.parts[-3:]):
            pass
        else:
            name = namer(fp)
            poscar = next(fp.parent.glob(fp.name.replace("OUTCAR", "CONTCAR")))
            symbols = contcar_parser(poscar)
            kpoints_file = file_finder(fp.parent, "KPOINTS")
            kpoints = get_kpoints(kpoints_file)
            incar_file = file_finder(fp.parent, "INCAR")
            incar = parse_incar(incar_file)

            configs = outcar_reader(fp=fp, symbols=symbols)
