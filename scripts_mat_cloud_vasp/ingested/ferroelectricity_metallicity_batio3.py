...
File notes
----------
Config set for no-doping has no outcar files
...
DATASET_FP = Path("data/interplay_between_ferroelectricity_and_metallicity_in_batio3")
DATASET_NAME = "ferroelectricity_and_metallicity_in_BaTiO3_JMCC2021"
LICENSE = "https://creativecommons.org/licenses/by/4.0/"

PUBLICATION = "http://doi.org/10.1039/D1TC01868J"
DATA_LINK = "https://doi.org/10.24435/materialscloud:f4-94"
# OTHER_LINKS = []

AUTHORS = ["Veronica F. Michel", "Tobias Esswein", "Nicola A. Spaldin"]
DATASET_DESC = (
    'Dataset for "Interplay between ferroelectricity and metallicity in BaTiO3", '
    "exploring properties of ferroelectric barium titanate (BaTiO3), including "
    "the effects of electron and hole doping. Includes configuration sets for unit "
    "cells and supercells of BaTiO3."
)
ELEMENTS = None
GLOB_STR = "OUTCAR"
...
CSS = [
    [
        f"{DATASET_NAME}_unit_cell_{elem[0]}",
        {"names": {"$regex": f"{elem[0]}"}},
        f"Configurations of {elem[1]} unit-cell BaTiO3 from {DATASET_NAME} dataset",
    ]
    for elem in [
        ("electron_doping", "electron-doped"),
        ("hole_doping", "hole-doped"),
        # ("no_doping", "undoped"),
    ]
]
CSS.extend(
    [
        [
            f"{DATASET_NAME}_supercell_{elem}",
            {"names": {"$regex": f"{elem}"}},
            f"Configurations of supercell BaTiO3 doped with {elem} from {DATASET_NAME} dataset",
        ]
        for elem in [
            "aluminum",
            "lanthanum",
...
def reader(filepath: Path):
    ...
    try:
        ...
        poscar = file_finder(filepath, "POSCAR")
