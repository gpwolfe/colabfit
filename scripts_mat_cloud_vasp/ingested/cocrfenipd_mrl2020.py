...
DATASET_FP = Path("data/origin_of_high_strength_in_the_cocrfenipd_high-entropy_alloy")
DATASET_NAME = "CoCrFeNiPd_MRL2020"
LICENSE = "https://creativecommons.org/licenses/by/4.0/"

PUBLICATION = "https://doi.org/10.24435/materialscloud:2020.0045/v1"
DATA_LINK = "https://doi.org/10.24435/materialscloud:2020.0045/v1"
# OTHER_LINKS = []

AUTHORS = ["Binglun Yin", "W. A. Curtin"]
DATASET_DESC = (
    'The dataset for "Origin of high strength in the CoCrFeNiPd high-entropy alloy", '
    "containing DFT-calculated values of the high-entropy alloy CoCrFeNiPd, created to "
    "explore the reasons behind experimental findings of the increased strength "
    "CoCrFeNiPd in comparison to CoCrFeNi."
)
ELEMENTS = None
GLOB_STR = "OUTCAR"
...
CSS = [
    [
        f"{DATASET_NAME}_Ni_1e-6",
        {"names": {"$regex": "Ni_EDIFF_1e-6__"}},
        "Configurations of Ni calculated with energy cutoff setting of 1e-6 from "
        f"{DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_Ni_1e-7",
        {"names": {"$regex": "Ni__"}},
        "Configurations of Ni calculated with energy cutoff setting of 1e-7 from "
        f"{DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_Ni_4e-8",
        {"names": {"$regex": "__Ni_EDIFF_4e-8__"}},
        "Configurations of Ni calculated with energy cutoff setting of 4e-8 from "
        f"{DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_CrFeCoNi",
        {"names": {"$regex": "CrFeCoNi_"}},
        f"Configurations of CrFeCoNi from {DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_CrFeCoNiPd",
        {"names": {"$regex": "CrFeCoNiPd_"}},
        f"Configurations of CrFeCoNiPd from {DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_Ni2Pd",
        {"names": {"$regex": "Ni2Pd_"}},
        f"Configurations of Ni2Pd from {DATASET_NAME} dataset",
    ],
    [
        f"{DATASET_NAME}_NiCoCr",
        {"names": {"$regex": "NiCoCr_"}},
        f"Configurations of NiCoCr from {DATASET_NAME} dataset",
    ],
]
