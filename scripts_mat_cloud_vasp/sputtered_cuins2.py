...
DATASET_FP = Path(
    "data/experimental_and_theoretical_study_of_stable_"
    "and_metastable_phases_in_sputtered_cuins2"
)
DATASET_NAME = "stable_and_metastable_phases_in_sputtered_CuInS2"
LICENSE = "CC-BY-4.0"

PUBLICATION = "http://doi.org/https://doi.org/10.1002/advs.202200848"
DATA_LINK = "https://doi.org/10.24435/materialscloud:5n-1e"
# OTHER_LINKS = []
...
DATASET_DESC = (
    "The chalcopyrite Cu(In,Ga)S2 has gained renewed interest in recent years due to "
    "its potential application in tandem solar cells. In this contribution, a "
    "combined theoretical and experimental approach is applied to investigate stable "
    "and metastable phases forming in sputtered CuInS2 (CIS) thin films. Ab initio "
    "calculations are performed to obtain formation energies, X-ray diffraction "
    "patterns, and Raman spectra of various CIS polytypes and related compounds. "
    "Multiple low-energy CIS structures with zinc-blende and wurtzite-derived "
    "lattices are identified and their XRD/Raman patterns are shown to contain many "
    "overlapping features, which could lead to misidentification unless the "
    "techniques are duly combined and analyzed. The results are verified against "
    "experimental XRD/Raman spectra measured on a series of CIS films with different "
    "compositions and treated at different temperatures, revealing the formation of "
    "several CIS polymorphs and secondary phases. The characteristic features and "
    "the mechanisms behind the formation of different phases are discussed with the "
    "focus on the thin-film photovoltaic application of CIS. The dataset contains "
    "structures and VASP output files used to derive the discussed trends. version 2"
)
ELEMENTS = None
GLOB_STR = "OUTCAR*"
...
def reader(filepath: Path):
    name = namer(filepath)
    poscar = file_finder(filepath.parent, "POSCAR*")
