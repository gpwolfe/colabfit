...
tar -xf data-set-2016-TiO2.tar.bz2 -C  <project_dir>/data/
...
DATASET_FP = Path("data/data-set-2016-TiO2/")
...
COORD_F_RE = re.compile(
    r"(?P<element>\w+)\s+(?P<x>\-?\d+\.\d+)\s+(?P<y>\-?\d+\.\d+)\s+(?P<z>\-?\d+\.\d+)"
...
def main(argv):
    ...
    metadata = {
        ...
        "total-energy": {"field": "total-energy", "units": "Ry"},
    }
    property_map = {
        "atomic-forces": [
            {
